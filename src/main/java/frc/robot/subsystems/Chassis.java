...
public class Chassis extends SubsystemBase{
    ...
    public Chassis(){     //"restarting" the variables = constructor
        
        //first we "tell" each motor which port it belongs to.
        //We have already "decided" that in the constants class:
        rightFront = new TalonFX(Constants.PORT_NUMBER_FRONTRIGHT);
        rightRear = new TalonFX(Constants.PORT_NUMBER_REARRIGHT);
        leftFront = new TalonFX(Constants.PORT_NUMBER_FRONTLEFT);
        leftRear = new TalonFX(Constants.PORT_NUMBER_REARLEFT);
